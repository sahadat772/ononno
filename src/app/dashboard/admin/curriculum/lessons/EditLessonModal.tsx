...
export default function EditLessonModal({ open, lesson, chapters, subjects, classes, onClose, onSuccess }: Props) {
    ...
    const [form, setForm] = useState({
        ...
        coinReward: lesson?.coin_reward ?? 5,
    ...
    const currentChapter = chapters.find(c => c.id === lesson?.chapter_id)
    const currentSubject = subjects.find(s => s.id === lesson?.subject_id)
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (!res.ok) {
            ...
            setError(data.error || 'Lesson আপডেট করা যায়নি।')
            return
        }
                    ...
                    >
                        ...
                        <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1.2fr,0.8fr]">
                            <div className="space-y-4">
                                ...
                                <div>
                                    <h2 className="text-3xl font-black text-white">Edit lesson details</h2>
                                    <p className="mt-2 text-sm text-gray-400">
                                        Update the lesson information below.
                                    </p>
                                </div>
                            </div>
                            <div className="rounded-3xl bg-slate-950/80 p-4 text-sm space-y-2">
                                <p className="font-semibold text-white">Current Info</p>
                                <p className="text-xs text-amber-300">Chapter: {currentChapter?.title_bn ?? '—'}</p>
                                <p className="text-xs text-violet-300">Subject: {currentSubject?.name_bn ?? '—'}</p>
                                <p className="text-xs text-blue-300">Class: {currentClass?.name ?? '—'}</p>
                            </div>
                        </div>
                        ...
                        <div className="max-h-[calc(100vh-12rem)] overflow-y-auto px-6 pb-6 sm:px-8 sm:pb-8">
                            <div className="grid gap-5">
                                ...
                                <div>
                                    ...
                                    <input className={inputClass} value={form.titleBn}
                                        onChange={(e) => setForm({ ...form, titleBn: e.target.value })} required />
                                </div>
