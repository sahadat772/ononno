...
export default function CurriculumDashboardClient({
  ...
  async function runSeed(force = false) {
    ...
    try {
      ...
      setSeedMsg(
        `✅ Seed সম্পন্ন — Class ${s?.classes ?? 0}, Subject ${s?.subjects ?? 0}, Chapter ${s?.chapters ?? 0}, Lesson ${s?.lessons ?? 0}। পেজ রিফ্রেশ করো।`,
      )
    } catch {
    ...
    <main className="min-h-screen bg-[#030711] px-3 py-5 font-sans text-[#f7f7ff] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-4 py-2.5 text-xs font-bold text-emerald-200 hover:bg-emerald-500/20 disabled:opacity-50"
            >
          ...
          >
            {seedMsg ? (
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p>
                  Curriculum DB খালি। Seed Baseline দিয়ে প্রাইমারি ১–৫ যোগ করো, পরে PDF import দিয়ে
                  real content।
                </p>
                <button
                  type="button"
                ...
                >
                  {seedLoading ? 'Seeding…' : 'এখনই Seed করো'}
                </button>
              </div>
            )}
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  <div
                    className={`grid size-14 place-items-center rounded-xl border ${tone.border} bg-white/3 ${tone.icon}`}
                  >
        ...
        <section className="relative overflow-hidden rounded-xl border border-slate-700/80 bg-linear-to-br from-[#0b1223] to-[#070b15] p-5">
          ...
          <p className="mt-1 text-xs text-slate-400">AI generates → Admin reviews → Publish → Student learns</p>
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {actionItems.map(({ title, subtitle, Icon, color, href, ...rest }) => {
              ...
              return (
                ...
                >
                  <div
                    className={`mx-auto grid size-16 place-items-center rounded-2xl bg-white/4 ${tone.icon}`}
                  >
        ...
        <section className="overflow-hidden rounded-xl border border-slate-700/80 bg-linear-to-br from-[#0b1223] to-[#070b15]">
          <div className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">Live hierarchy — click a class to expand subjects</p>
            </div>
                ...
                <div key={s.id} className="flex flex-wrap items-center justify-between gap-2 px-5 py-3 text-sm">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {s.source_status} · {s.workflow_status} · {s.total_chapters} ch · {s.total_lessons} lessons
                    </p>
                  </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4 p-4">
                  <div className={`grid size-12 place-items-center rounded-xl border text-xl ${tone.border} ${tone.icon}`}>
                    {symbol}
                  </div>
                  <div>
...
function TreeRow({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-white/5">
      {!leaf ? (
      ...
      <span className={`flex-1 text-sm ${root ? 'font-bold' : 'font-medium'}`}>{label}</span>
      <span className="text-xs text-slate-500">{meta}</span>
      {href && (
        <Link href={href} className="text-xs text-violet-300 hover:text-violet-200">
          Open
        </Link>
      )}
    </div>
