        ...
        <main className="min-h-screen bg-[#0a0a1a] text-white">
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -60 }}
                        className={`fixed top-4 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-2xl text-sm font-semibold shadow-2xl ${toast.type === 'success'
            ...
            <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-[#0a0a1a]/90 backdrop-blur-xl px-4 py-3">
                <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
                        ...
                        {selectedSubject && (
                            <>
                                ...
                                <button
                                    ...
                                    className={`transition-colors truncate max-w-[120px] ${view === 'chapters' ? 'text-white font-semibold' : 'text-gray-400 hover:text-white'}`}
                                >
                    ...
                    <motion.button
                        ...
                        className="shrink-0 bg-gradient-to-r from-violet-600 to-purple-600 text-white px-4 py-2 rounded-xl text-sm font-semibold shadow-lg shadow-purple-900/40"
                    >
            ...
            <div className="max-w-6xl mx-auto px-4 pt-20 pb-12">
                                        ...
                                        >
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex flex-wrap gap-1.5">
                                                    <span className={`text-xs px-2 py-0.5 rounded-full bg-gradient-to-r ${cat?.color} text-white font-medium`}>
                                                        {cat?.icon} {cat?.label}
                                                    </span>
                                            ...
                                            <button
                                                ...
                                                className="w-full mt-2 py-2.5 rounded-xl bg-gradient-to-r from-violet-600/20 to-purple-600/20 border border-violet-500/30 text-violet-400 text-sm font-semibold hover:from-violet-600/30 hover:to-purple-600/30 transition-all flex items-center justify-center gap-2"
                                            >
                ...
                {view === 'chapters' && selectedSubject && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        {/* Subject header */}
                        <div className="mb-6 mt-4 rounded-2xl bg-gradient-to-r from-violet-600/10 to-purple-600/10 border border-violet-500/20 p-5">
                            <div className="flex items-center gap-3">
                                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${catInfo(selectedSubject.category)?.color} flex items-center justify-center text-2xl`}>
                        ...
                        {loading ? <LoadingGrid /> : chapters.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {chapters.map((chapter, i) => (
                                    ...
                                    >
                                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-600 to-purple-600 flex items-center justify-center text-white font-bold shrink-0">
                                            {i + 1}
                                        </div>
                ...
                {view === 'lessons' && selectedChapter && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        {/* Chapter header */}
                        <div className="mb-6 mt-4 rounded-2xl bg-gradient-to-r from-blue-600/10 to-cyan-600/10 border border-blue-500/20 p-5">
                            <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-3">
                                {lessons.map((lesson, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-lg shrink-0">
                                                {lt?.icon || '📝'}
                                            </div>
                                            <div className="flex-1 min-w-0">
                ...
                <form onSubmit={saveSubject} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <FormField label="Order Index">
                            <input type="number" value={subjectForm.order_index} onChange={e => setSubjectForm({ ...subjectForm, order_index: +e.target.value })} className={inputCls} min={0} />
                        </FormField>
                        <FormField label=" ">
            ...
            <Modal show={showChapterForm} onClose={() => setShowChapterForm(false)} title={`নতুন Chapter — ${selectedSubject?.name_bn}`}>
                <form onSubmit={saveChapter} className="space-y-4">
                    ...
                    <FormField label="Order Index">
                        <input type="number" value={chapterForm.order_index} onChange={e => setChapterForm({ ...chapterForm, order_index: +e.target.value })} className={inputCls} min={0} />
                    </FormField>
                    <FormField label="বিবরণ (ঐচ্ছিক)">
                ...
                <form onSubmit={saveLesson} className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        ...
                        <FormField label="সময় (মিনিট)">
                            <input type="number" value={lessonForm.duration_minutes} onChange={e => setLessonForm({ ...lessonForm, duration_minutes: +e.target.value })} className={inputCls} min={1} max={120} />
                        </FormField>
                        <FormField label="XP পুরস্কার">
                            <input type="number" value={lessonForm.xp_reward} onChange={e => setLessonForm({ ...lessonForm, xp_reward: +e.target.value })} className={inputCls} min={1} />
                        </FormField>
                    </div>
                    <FormField label="Order Index">
                        <input type="number" value={lessonForm.order_index} onChange={e => setLessonForm({ ...lessonForm, order_index: +e.target.value })} className={inputCls} min={0} />
                    </FormField>
                    <FormField label="Content (ঐচ্ছিক — AI generate করবে)">
                        <textarea value={lessonForm.content} onChange={e => setLessonForm({ ...lessonForm, content: e.target.value })}
                            placeholder="Lesson এর মূল বিষয়বস্তু লেখো, অথবা ফাঁকা রাখো — AI নিজেই তৈরি করবে..." rows={4} className={inputCls + ' resize-none'} />
                    </FormField>
    ...
    return (
        <div className="flex gap-3 pt-2">
            ...
            <motion.button type="submit" disabled={saving}
                ...
                className="flex-1 bg-gradient-to-r from-violet-600 to-purple-600 text-white py-3 rounded-xl text-sm font-semibold shadow-lg disabled:opacity-50">
    ...
    return (
        <div className="text-center py-20">
            ...
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onAdd}
                className="bg-gradient-to-r from-violet-600 to-purple-600 text-white px-6 py-3 rounded-xl text-sm font-semibold shadow-lg">
                + {desc}
            </motion.button>
        </div>
    )
